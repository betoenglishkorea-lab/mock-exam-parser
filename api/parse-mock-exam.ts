...
## ⚠️ 핵심 원칙 (가장 중요 - 반드시 준수)

**당신은 문제를 푸는 것이 아니라, PDF 원본을 그대로 복원하는 것입니다.**

### 🚫 절대 금지 (위반 시 심각한 오류)
1. **번역 금지**: 영어 지문을 한국어로 번역하지 마세요. passage 필드에는 영어 원문만!
2. **빈칸 채우기 금지**: 빈칸은 ________로 유지
3. **단락 순서 변경 금지**: (A), (B), (C) 순서 그대로
4. **정답 추론 금지**: 문제를 풀지 마세요

### ✅ 반드시 지킬 것
- PDF에 있는 그대로 추출 (영어는 영어로, 한국어는 한국어로)
- passage 필드 = 영어 원문 지문
- model_translation 필드 = 한국어 해석 (PDF에 있는 경우만)
...
- **단락 순서 변경 금지**: (A), (B), (C), (D) 순서를 절대 바꾸지 마세요
...
### ⚠️ 빈칸 처리 (매우 중요 - 반드시 준수)

**핵심: 빈칸은 절대로 채우지 마세요. 문제를 푸는 것이 아닙니다.**

**빈칸 표시 규칙:**
- PDF에서 빈칸(_____, blank, 공백)이 있으면 → ________ (언더스코어 8개)로 표시
...
- 빈칸에 선지 내용 삽입 금지
- 빈칸에 추론한 내용 삽입 금지
- 빈칸을 대괄호 []로 감싸서 내용 넣기 금지

**예시:**
| 상황 | 잘못된 처리 | 올바른 처리 |
|------|------------|------------|
| 빈칸 문제 | solutions [hides a steady accumulation] | solutions ________ |
| 빈칸 문제 | the ability to ________ [make decisions] | the ability to ________ |
| 요약문 빈칸 | (A) [increased] (B) [decreased] | (A) ________ (B) ________ |

**빈칸 vs 밑줄 구분:**
- 빈칸: 내용이 비어있음 → ________로 유지
- 밑줄: 이미 텍스트가 있음 → [텍스트]로 감싸기
...
### ⚠️ 핵심: PDF 원본 복원 (문제 풀이 금지)
- 연계문제(장문독해)에서도 **(A), (B), (C), (D) 단락 순서를 PDF 원본 그대로** 유지
- 정답이 "(C)-(A)-(B)"라고 해서 단락을 재배치하면 **절대 안 됨**
- PDF에 (A) → (B) → (C) → (D) 순서로 나와 있으면 그대로 출력
...
4. **단락 순서 유지 (매우 중요)**:
   - (A), (B), (C), (D) 단락이 있으면 PDF에 나온 순서 그대로 유지
   - 정답 순서로 재배치 **금지**
   - 예: PDF가 (A)→(B)→(C) 순이면 passage도 (A)→(B)→(C) 순

5. **밑줄 표시 유지**: 연계문제의 모든 문제에서 동일한 밑줄 표시 유지
...
### ⚠️ 처리 규칙 (매우 중요 - 반드시 준수)

**핵심: PDF 원본 복원 (문제 풀이 금지)**
- 당신은 문제를 푸는 것이 **아닙니다**
- PDF의 내용을 **그대로** 추출하는 것입니다
- 정답을 보고 단락을 재배치하면 **절대 안 됩니다**
...
**단락 순서 규칙:**
- **(A), (B), (C) 순서를 절대로 바꾸지 말 것**
- PDF에 (A) → (B) → (C) 순서로 인쇄되어 있으면 그대로 (A) → (B) → (C)
- 정답이 "(B)-(A)-(C)"라고 해서 (B) 단락을 먼저 배치하면 **안 됨**
- 정답이 "(C)-(B)-(A)"라고 해서 (C) 단락을 먼저 배치하면 **안 됨**
- (A), (B), (C) 표시는 그대로 유지
...
## 🔍 최종 검토 체크리스트 (출력 전 반드시 확인)

**각 문항을 출력하기 전에 아래 항목을 점검하세요:**

1. **passage 필드가 영어인가?**
   - ❌ 잘못됨: "그는 학교에 갔다" (한국어 번역)
   - ✅ 올바름: "He went to school" (영어 원문)

2. **빈칸이 ________로 유지되어 있는가?**
   - ❌ 잘못됨: "the ability to [make decisions]"
   - ✅ 올바름: "the ability to ________"

3. **(A), (B), (C) 순서가 PDF 원본 그대로인가?**
   - ❌ 잘못됨: 정답 순서로 재배치
   - ✅ 올바름: PDF에 나온 순서 그대로

4. **model_translation에만 한국어 해석이 있는가?**
   - passage = 영어 원문
   - model_translation = 한국어 해석

**위 항목 중 하나라도 위반하면 심각한 오류입니다. 반드시 수정 후 출력하세요.**
`;
